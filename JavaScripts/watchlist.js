...
function removeFromWatchlist(containerId, popupId) {
    ...
    setTimeout(function () {
        ...
        const updatedWatchlist = watchlist.filter(movie => movie.title !== container.querySelector('.movie-card-title').innerText);
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    watchlistContainer.innerHTML = movieCardsHTML;

    // alert(combinedWatchlist.length);

    const noWatchlistContainer = document.querySelector('.no-watchlist-container');
    noWatchlistContainer.style.display = combinedWatchlist.length === 0 ? 'block' : 'none';
});
